(() => {
  ...
  var Interpreter = /* @__PURE__ */ function() {
    ...
    Interpreter2.prototype.spawnMachine = function(machine2, options) {
      ...
      var childService = new Interpreter2(machine2, __assign(__assign({}, this.options), {
        parent: this,
    ...
    invoke: {
      src: () => (send3) => {
        const listener = (event2) => {
          try {
            const ourEvent = JSON.parse(event2.data);
            send3(ourEvent);
          } catch (e) {
            console.warn(e);
          }
    ...
    states: {
      waitingForFirstContact: {
        on: {
          RECEIVE_SERVICE: {
            target: "hasService",
            actions: assign2((_context, event2) => {
              return {
                config: event2.config,
                index: event2.index,
                uri: event2.uri,
                layoutString: event2.layoutString,
                token: event2.token,
                implementations: event2.implementations,
                baseUrl: event2.baseUrl
              };
            }),
            internal: false
          }
        ...
        invoke: {
          src: (context) => () => {
            ...
            if (!iframe || iframe.src)
              return;
            iframe.src = `${context.baseUrl}/registry/editor/from-url?runningInStatelyExtension=true&config=${(0, import_lz_string.compressToEncodedURIComponent)(JSON.stringify(context.config))}${context.layoutString ? `&layout=${context.layoutString}` : ""}&implementations=${(0, import_lz_string.compressToEncodedURIComponent)(JSON.stringify(context.implementations))}${getTokenHash(context.token)}`;
          }
        },
        on: {
          RECEIVE_SERVICE: {
            actions: [
              assign2((_context, event2) => {
                return {
                  config: event2.config,
                  index: event2.index,
                  uri: event2.uri,
          ...
          RECEIVE_CONFIG_UPDATE_FROM_VSCODE: {
            cond: (ctx, event2) => {
              return event2.uri === ctx.uri && ctx.index === event2.index;
            },
            actions: [
              assign2((ctx, event2) => {
                return {
  ...
  var getTokenHash = (tokenInfo) => {
    return `#access_token=${tokenInfo.token}&expires_in=${(tokenInfo.expiresAt - Date.now() / 1e3).toFixed(0)}&provider_token=${tokenInfo.providerToken}&refresh_token=${tokenInfo.refreshToken}&token_type=bearer`;
  };
})();
/*! *****************************************************************************
